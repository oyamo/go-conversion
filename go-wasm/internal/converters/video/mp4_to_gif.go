...
func convertMp4ToGif(data []byte) ([]byte, error) {
	...
	if (!window.mp4ToGif) {
		window.mp4ToGif = function(arrayBuffer) {
	...
	jsPromise := js.Global().Call("mp4ToGif", uint8Array)
	...
	reject := js.FuncOf(func(this js.Value, args []js.Value) any {
		errCh <- fmt.Errorf("mp4 to gif failed: %s", args[0].String())
		return nil
	})
